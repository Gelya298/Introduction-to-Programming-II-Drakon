...
function showHideIDC(page)
{
	...
	else
	{
		...
		IDReplaceHtml($id('idc-showcomments-link'), "Показать комментарии");
		$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
		$id('idc-showcomments-link').style.display = showCommentsStyle;
		IDReplaceHtml($id('idc-postcomment-link'), "Показать комментарии");
...
function showIDC(page)
{
	...
	showCommentsStyle = $id('idc-showcomments-link').style.display;
	postCommentStyle = $id('idc-postcomment-link').style.display;
	IDReplaceHtml($id('idc-showcomments-link'), "Скрыть комментарии");
	$id('idc-showcomments-link').href = 'javascript:showHideIDC();';
	$id('idc-showcomments-link').style.display = showCommentsStyle;
	IDReplaceHtml($id('idc-postcomment-link'), "Скрыть комментарии");
	$id('idc-postcomment-link').href = 'javascript:showHideIDC();';
	$id('idc-postcomment-link').style.display = postCommentStyle;
	if($id('idc-footer'))
...
function voteComment(commentid, vote) {
	...
	{
		showMsgBox("Извините", "<p>Вы уже проголосовали за этот комментарий.</p>", 0, listObj);
		return;
	}
	
	if(commentObj.comments[commentid] && (commentObj.comments[commentid].status==2 || commentObj.comments[commentid].status==6))
	{
		showMsgBox("Извините", "<p>Голосование за удаленный комментарий невозможно.</p>", 0, listObj);
		return;
	}
	...
	{
		showMsgBox("Извините", "<p>Вы уже проголосовали за этот комментарий.</p>", 0, $id("IDComment"+commentid));
		return;
	}
...
{	
	IDReplaceHtml($id("adminOptions"), 'Загружаем...');
	$id("adminOptions").href = "javascript:void(0);";		
	
	if(!commentObj.curUser.isLoggedIn)
	{
		showMsgBox("Извините", "<p>Авторизуйтесь, чтобы изменять настройки.</p>", 0, listObj);
		return;
	}
	//changeDisabledLink();	
...
function postComment(src)
{
	...
	if(src==0) //new thread	
	{
		if( commentObj.postUsingTwitter ) {
			...
			{
				showMsgBox("Извините", "<p>Ваш комментарий получился слишком длинным. Попробуйте разбить его на несколько.");
				return;
			}
				
			IDReplaceHtml($id("IDNewThreadSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Отправляем..." />');
		...
		{
			if($id('IDCommentNewThreadText').value=='Введите текст здесь!' || $id('IDCommentNewThreadText').value.length==0)
			{
				showMsgBox("Извините", "<p>Введите текст комментария перед тем, как опубликовать его.</p>", 0);
				return;
			}
			...
			if(commentObj.newthreadType == 4) //signup
			{
				...
				{
					showMsgBox("Извините", "<p>Необходим Ваш адрес OpenID.</p>", 0);
					return;
				}
		...
		//Check for defaults
		if($id('IDCommentNewThreadText').value=='Введите текст здесь!')
		{
			showMsgBox("Извините", "<p>Введите текст комментария перед тем, как опубликовать его.</p>", 0);
			return;
		}		
		...
		{
			showMsgBox("Извините", "<p>Укажите корректный email адрес, чтобы зарегистрироваться</p>", 0);
			return;
		}
		
		if($id('txtNameNewThread').value == '' && commentObj.newthreadType == 0)
		{
			showMsgBox("Извините", "<p>Пожалуйста, укажите свое имя и повторите попытку</p>", 0);
			return;
		}

		/*if($id('txtNameNewThread').value.indexOf('&')>0)
		{
			showMsgBox("Извините", "<p>Ваше имя не должно содержать символ "&".</p>", 0);
			return;
		}*/			
	...
	else
	{
		if( commentObj.postUsingTwitter ) {
			...
			IDReplaceHtml($id("IDReplyDivSubmitLI"), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Отправляем..." />');		
			var requestObj = new buildRequestObj(theStr, 0, null, connectionErr);
			xs.make_request(requestObj);
		...
		{
			if($id('txtComment').value=='Введите текст здесь!' || $id('txtComment').value.length==0)
			{
				showMsgBox("Извините", "<p>Введите текст комментария перед тем, как опубликовать его.</p>", 0);
				return;
			}
		...
		//Check for defaults
		if($id('txtComment').value=='Введите текст здесь!')
		{
			showMsgBox("Извините", "<p>Введите текст комментария перед тем, как опубликовать его.</p>", 0);
			return;
		}		
...
{
	IDReplaceHtml($id("IDReplyDivSubmitLI"), '<a class="idc-btn_l-secondary" href="javascript: hideReply();"><span></span><span class="idc-r">Отменить</span></a><a class="idc-btn_l" href="javascript: postComment(1);"><span></span><span class="idc-r"><strong>Оставить комментарий</strong></span></a>');
	IDReplaceHtml($id("IDNewThreadSubmitLI"), '<a class="idc-btn_l" href="javascript: postComment(0);"><span></span><span class="idc-r"><strong>Оставить комментарий</strong></span></a>');
	showMsgBox("Извините","<p>Произошла ошибка соединения. Время соединения истекло.</p>", 2);	
};
...
function showReply(commentid) {
	...
	if( commentObj.parentid )
		IDReplaceHtml($id("IDCommentPostReplyLink" + commentObj.parentid), "<a class='idc-btn_s' href='javascript: showReply(" + commentObj.parentid + ")'><span><input type='hidden' id='reqUsersOn' value='no' /></span><span class='idc-r'>Ответить</span></a>");
		
	IDReplaceHtml($id("IDCommentPostReplyLink"+commentid), "<span class='idc-btn_s'><span></span><span class='idc-r'>Публикуем...</span></span>");

		
	createCookie("IDReplyCommentId", commentid, 1);
	
	if($id("IDCommentReplyName") && commentObj.comments[commentid].displayName)
		IDReplaceHtml($id("IDCommentReplyName"), "Ответ для #user# ".replace(/#user#/, commentObj.comments[commentid].displayName));
...
{	
	...
	p.className="idc-bottom";
		...
		IDReplaceHtml(link2, "<span></span><span class='idc-r'>Закрыть сообщение</span>");
...
{	
	...
	var h6 = $newEl("h6");
	var type = 0;
	var header = 'Пожаловаться на комментарий';
	IDReplaceHtml(h6, header);
	var pText = $newEl('div');
	var text = '<p>'+$id('IDCustomReportTxt').innerHTML+'</p><p>Причина жалобы:</p><textarea id="IDCCommentAdditional" class="idc-text_noresize" value="" style="width:90%;"></textarea><p><a href="javascript: reportThisComment('+commentId+');" class="idc-btn_s"><span></span><span class="idc-r">Пожаловаться на это!</span></a></p>';
...
function showReputationWhy() {
	showMsgBox("Индикатор репутации IntenseDebate", "<p>Индикатор репутации - это оценка всех предыдущих комментариев определенного пользователя, сделанная другими пользователями. По этому показателю можно понять, оставлен ли комментарий уважаемым автором или нет.</p> <img src='"+IDHost+"/images/id-rep_scale.png' alt='Шкала репутации' />", 0 );
};

function showForgotPassword()
{	
	showMsgBox("Forgot your password?", "<form><label for=\"txtResolveEmail\">Пожалуйста, введите свой email:</label><input type=\"text\" class=\"idc-text\" id=\"txtResolveEmail\" style=\"color:black\" onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><a href=\"javascript: forgotPassword();\" class=\"idc-btn_s\"><span></span><span class='idc-r'>Отправьте мне мой пароль!</span></a></form>", 0);
...
{	
	showMsgBox("Есть, что сказать? Отлично!", "<form><label for='txtFeedbackEmail'>Email:</label><input type='text' id='txtFeedbackEmail' name='txtEmail' class='idc-text' onfocus=\"txtOnFocus(this, '');\" onblur=\"txtOnBlur(this, '');\" /><textarea name='txtFeedback' class='idc-text' id='txtFeedback' value='Feedback' onfocus=\"txtOnFocus(this, 'Feedback');\" onblur=\"txtOnBlur(this, 'Feedback');\"></textarea><a href='javascript: sendFeedback()' class='idc-btn_s'><span></span><span class='idc-r'>Отправить сообщение</span></a></form>", 0);
...
{
	showMsgBox("Зачем регистрироваться?", "<p><ul><li>Объединяйтесь с людьми, которые комментируют Ваши любимые блоги</li><li>Продвигайте себя, публикуя ссылки на свой профиль в Facebook, Твиттер и т.п.</li><li>Привлеките посетителей к своему блогу или сайту</li><li>Разрешить друзьям отслеживать Ваши комментарии во всех блогах с использованием ID комментариев</li></ul></p>", 0);
};
...
function showSignupNewThread()
{
    ...
    //$id("IDCommentsNewThreadListItem2").innerHTML='<h6><a href="javascript: showLoginNewThread();">Войти</a></h6>';
...
function showSignupReply()
{
	...
	//$id("IDCommentReplyLoginLink").innerHTML = '<h6><a href="javascript: showLoginReply();">Войти</a></h6>';	
...
function showSignupOpenIDReply()
{
	...
	//$id("IDCommentReplyLoginLink").innerHTML = '<h6><a href="javascript: showSignupReply();">Вернуться</a></h6>';
	//$id("IDPostReplyUseOpenID").style.display="none";
...
function collapseThread(commentid) {
	if($id("IDCommentCollapseLink" + commentid) && $id("IDCommentCollapseLink" + commentid).className == "idc-collapselink") {
		...
		if( $id("IDCommentSubThread" + commentid).childNodes.length == 0 ) { //content not loaded yet
			...
			IDReplaceHtml($id("IDCommentSubThread" + commentid), "<div class='idc-thread_loading'><img src='http://s.intensedebate.com/images/ajax-loader.gif' alt='Загружаем...' /> Загружаем...</div>");
...
function IDeditComment(commentid)
{
	...
	{
		showMsgBox("Извините", "<p>Авторизуйтесь, чтобы изменить комментарий.</p>", 0, null);		
		return;
	}
	
	if(!commentObj.comments[commentid] || !commentObj.comments[commentid].commentid)
	{
		showMsgBox("Извините", "<p>Этот комментарий не существует...</p>", 0, null);		
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<a href="javascript: IDcancelEditComment('+commentid+');" class="idc-btn_l-secondary"><span></span><span class="idc-r">Отменить</span></a><a href="javascript: IDsaveComment('+commentid+');" class="idc-btn_l"><span></span><span class="idc-r"><strong>Сохранить</strong></span></a>');
...
function IDsaveComment(commentid)
{
	if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
	{
		showMsgBox("Извините", "<p>Авторизуйтесь, чтобы удалить комментарий.</p>", 0, null);		
		return;
	}
	...
	IDReplaceHtml($id("IDCommentCancelSave"+commentid), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Отправляем..." />');
...
{
	var answer = confirm("Вы уверены, что хотите удалить этот комментарий?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
		...
		if(type<0 || type>1)
		{
			showMsgBox("Извините", "<p>Некорректный тип.</p>", 0, null);		
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkDelete"+commentid), 'Удаляем...');
...
{
	var answer = confirm("Вы уверены, что хотите удалить этот трэкбэк?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Извините", "<p>Авторизуйтесь, чтобы удалить трэкбэк.</p>", 0, null);		
			return;
		}
		...
		{
			IDReplaceHtml($id("IDTrackBackDeleteLink"+trackbackid), 'Удаляем...');
		...
		{
			showMsgBox("Извините", "<p>Этот трэкбэк не существует...</p>", 0, null);
			return;		
		}
	}	
}
...
{
	var answer = confirm("Вы уверены, что хотите заблокировать этот IP?")
	if (answer)
	{
		if(!commentObj.curUser.userid || commentObj.curUser.userid <= 0)
		{
			showMsgBox("Извините", "<p>Авторизуйтесь, чтобы заблокировать IP.</p>", 0, null);		
			return;
		}
		...
		IDReplaceHtml($id("IDCommentPostReplyLinkBan"+commentid), 'Блокируем...');
...
function IDCSubscribeByEmail(type)
{
	...
	IDReplaceHtml($id('IDCSubscribeSubmit'+type), '<img src="http://s.intensedebate.com/images/ajax-loader.gif" alt="Отправляем..." />');
...
IDC.user_menu = {
    ...
    "add_friend": function ( uid ) {
            if ( commentObj && commentObj.curUser && !commentObj.curUser.isLoggedIn ) {
                    showMsgBox("Извините", "<p>Авторизуйтесь, чтобы добавлять друзей.</p>", 0, null);
                    return;
            }
...
IDC.user_menu.clear_cache();		var comment_array = null;
	...
	var IDCounted;
	_qoptions = [{ labels:"languages.ru_RU,idc", qacct:"p-94D6e1NDscLvI"},{qacct:"p-18-mFEk4J448M", labels:"type.intensedebate.embed"}];
	IDcountIT();
}
...
function IDcountIT() {
	...
	newScript.src  = 'http://edge.quantserve.com/quant.js';
